...
class LiteratureReviewGenerator:
    def __init__(self, query, api_key: str, ablation_study = ''):
        ...
        self.CHAT_AGENT_MODEL = "gpt-4o-mini"
        self.ADVANCED_CHAT_AGENT_MODEL = "gpt-4o"
        self.papers_data = []
    ...
    def evaluate_section_quality(self, section_title: str, section_content: str, 
        ...
        try:
            ...
            #     )
            # )
            response = self.chat_agent.chat(evaluation_prompt, model=self.ADVANCED_CHAT_AGENT_MODEL, temperature=0.2, provider='gpt')
    ...
    def filter_additional_papers_with_llm_check(self, subsection_title, subsection_focus, current_content, weaknesses, retrieved_papers):
        ...
        try:
            ...
            #     )
            # )
            response = self.chat_agent.chat(prompt, model = self.CHAT_AGENT_MODEL, temperature=0.4, provider='gpt')
            return self.parse_rag_result(response)
    ...
    def improve_section_with_additional_papers(self, section_title: str, current_content: str,
        ...
        try:
            response = self.chat_agent.chat(improvement_prompt, model = self.CHAT_AGENT_MODEL, temperature=0.4, provider='gpt')
            return response
    ...
    def write_initial_subsection(self, subsection_title: str, subsection_focus: str, 
        ...
        try:
            ...
            #     )
            # )
            response = self.chat_agent.chat(subsection_prompt, model = self.CHAT_AGENT_MODEL, temperature=0.4, provider='gpt')
            return self.parse_remove_think(response)
        except Exception as e:
...
# region write initial section
    def write_initial_section(self, section_title: str, processed_papers: List[Dict], 
        ...
        try:
            ...
            #     )
            # )
            response = self.chat_agent.chat(section_prompt, model = self.CHAT_AGENT_MODEL, temperature=0.4, provider='gpt')
            return response
        except Exception as e:
    ...
    def write_initial_section_overview(self, section_title: str, section_focus: str, 
        ...
        try:
            ...
            #     )
            # )
            response = self.chat_agent.chat(section_overview_prompt, model = self.CHAT_AGENT_MODEL, temperature=0.4, provider='gpt')
            return response
        except Exception as e:
