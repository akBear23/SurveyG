...
class LiteratureReviewGenerator:
    def __init__(self, query, api_key: str):
        ...
        self.query = query
        self.save_dir = f"paper_data/{self.query.replace(' ', '_').replace(':', '')}/literature_review_output_without_rag"
        os.makedirs(self.save_dir, exist_ok=True)
        try:
            shutil.copy(f"paper_data/{self.query.replace(' ', '_').replace(':', '')}/literature_review_output/survey_outline.json", f"{self.save_dir}/survey_outline.json")
            print("File outline.json copied successfully.")
