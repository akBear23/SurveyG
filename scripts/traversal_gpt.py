...
def llm_call_with_retry(prompt, placeholder="LLM summary placeholder", temperature=0.3, model=Config.GPT_MODEL):
    ...
    while finish_generated == False and tries < 3:
        tries += 1
        try:
            summary = chat_agent.chat(prompt, model=model, temperature=temperature)
...
"""
    ...
    chat_agent = ChatAgent()
    try:
        response = chat_agent.chat(base_prompt, model=Config.GPT_MODEL, temperature=Config.LLM_TEMPERATURE)
...
def evaluate_outline(query, outline_text, save_dir):
    ...
    evaluation = llm_call_with_retry(evaluation_prompt, model=Config.ADVANCED_GPT_MODEL)
...
def main():
    ...
    save_outline_dir = f"paper_data/{query.replace(' ', '_').replace(':', '')}/literature_review_output{ablation_study}"
    os.makedirs(save_outline_dir, exist_ok=True)
    survey_outline_path = f"{save_outline_dir}/survey_outline{ablation_study}.json"
    ...
    if ablation_study == '_without_community':
        layer_method_group_json = {}
        communities_summary = {}
    if ablation_study == '_without_bfs':
        develop_direction = {}
