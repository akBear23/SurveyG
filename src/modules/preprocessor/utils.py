...
def parse_arguments_for_preprocessor() -> ArgsNamespace:
    ...
    parser.add_argument(
        ...
        default=None,
        help="LLM provider to use (openai or gemini). If not specified, uses config default.",
    )
    return parser.parse_args()


def parse_arguments_for_integration_test() -> ArgsNamespace:
...
def set_provider_if_specified(args, restore_on_exit=True):
    """
    Set the LLM provider if specified in arguments.
    
    Args:
        args: Parsed arguments containing provider option
        restore_on_exit: Whether to return original provider for restoration
                        Set to False for scripts that don't need restoration
    
    Returns:
        Original provider if restore_on_exit=True and provider was changed, None otherwise
    """
    if hasattr(args, 'provider') and args.provider:
        ...
        return original_provider if restore_on_exit else None
    return None
